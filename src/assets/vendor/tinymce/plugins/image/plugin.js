...
(function () {
    ...
    const eq = t => a => t === a;
    const is = (value, constructor) => isObject(value) &&
    ...
    const isPlaceholderImage = imgElm => imgElm.nodeName === 'IMG' &&
    ...
    const getIsDecorative = image => DOM.getAttrib(image, 'alt').length === 0 && DOM.getAttrib(image, 'role') ===
        'presentation';
    const getAlt = image => {
    ...
    const write = (normalizeCss, newData, image) => {
        ...
        updateProp(image, oldData, newData, 'style',
    ...
    const splitTextBlock = (editor, figure) => {
        ...
        const textBlockElements = filter(editor.schema.getTextBlockElements(),
            (_, parentElm) => !editor.schema.isValidChild(parentElm, 'figure'));
        const textBlock = dom.getParent(figure.parentNode, node => hasNonNullableKey(textBlockElements, node.nodeName),
            editor.getBody());
        if (textBlock) {
    ...
    const register = editor => {
        editor.ui.registry.addToggleButton('image', {
            ...
            onSetup: buttonApi => {
                ...
                return editor.selection.selectorChangedWithUnbind(
                    'img:not([data-mce-object]):not([data-mce-placeholder]),figure.image', buttonApi.setActive).unbind;
            }
        });
        ...
        editor.ui.registry.addContextMenu('image', {
            update: element => isFigure(element) || isImage(element) && !isPlaceholderImage(element)
                ? ['image']
                : []
        });
    };
