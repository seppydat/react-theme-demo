...
<p>When a tabbed dialog is opened, the first button in the tab menu is focused. Pressing tab will navigate to the first interactive component in that tab, and will cycle through the tab\u2019s components, the footer buttons, then back to the tab button. To switch to another tab, focus the tab button for the current tab, then use the arrow keys to cycle through the tab buttons.</p>`;
    ...
    const tab$1 = editor => {
        const availablePlugins = () => {
            ...
            return '<div data-mce-tabstop="1" tabindex="-1">' + '<p><b>' + global$1.translate('Premium plugins:') +
                '</b></p>' + '<ul>' + premiumPluginList +
                ...
                global$1.translate('Learn more...') + '</a></li>' + '</ul>' + '</div>';
        };
        ...
        const pluginLister = editor => {
            ...
            const sortedPluginData = sort(map(pluginKeys, k => getPluginData(editor, k)),
    ...
    const tab = () => {
        ...
        const changeLogLink = '<a href="https://www.tiny.cloud/docs/tinymce/6/changelog/?utm_campaign=editor_referral&utm_medium=help_dialog&utm_source=tinymce" rel="noopener" target="_blank">TinyMCE ' +
            version + '</a>';
        const htmlPanel = {
            type: 'htmlpanel',
    ...
    const parseCustomTabs = (editor, customTabs) => {
        ...
        return Optional.from(getHelpTabs(editor)).
            fold(() => getNamesFromTabs(tabs), tabsFromSettings => parseHelpTabsSetting(tabsFromSettings, tabs));
    };
