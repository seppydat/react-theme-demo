...
(function () {
        ...
        for (let node = rootNode; node; node = node.walk()) {
            if (node.type === 1) {
                ...
                if (node.attr('data-ephox-embed-iri')) {
                    ...
                    if (name === 'iframe' || name === 'object' || name === 'embed' || name === 'video' || name ===
                        'audio') {
                        if (!data.type) {
                            data.type = name;
                        }
    ...
    const getIframeHtml = (data, iframeTemplateCallback) => {
        if (iframeTemplateCallback) {
            ...
            return '<iframe src="' + data.source + '" width="' + data.width + '" height="' + data.height + '"' +
                allowFullscreen + '></iframe>';
        }
    };
    const getFlashHtml = data => {
        let html = '<object data="' + data.source + '" width="' + data.width + '" height="' + data.height +
    ...
    const getVideoHtml = (data, videoTemplateCallback) => {
        if (videoTemplateCallback) {
            ...
            return '<video width="' + data.width + '" height="' + data.height + '"' +
                ...
                data.source + '"' + (data.sourcemime ? ' type="' + data.sourcemime + '"' : '') + ' />\n' +
    ...
    const isMediaElement = element => element.hasAttribute('data-mce-object') ||
    ...
    const getEmbedHtml = (editor, data) => {
        ...
        return embedHandler ? embedPromise(data, loadedData(editor), embedHandler) : defaultPromise(data,
            loadedData(editor));
    };
    ...
    const getValue = (data, metaData, sourceInput) => prop => {
        ...
        const getFromValueFirst = () => getFromData().
            bind(child => isObject(child) ? getNonEmptyValue(child).orThunk(getFromMetaData) : getFromMetaData().
                orThunk(() => Optional.from(child)));
        const getFromMetaFirst = () => getFromMetaData().
            orThunk(
    ...
    const submitForm = (prevData, newData, editor) => {
        var _a;
        newData.embed = updateHtml((_a = newData.embed) !== null && _a !== void 0 ? _a : '', newData, false,
            editor.schema);
        ...
        editor.ui.registry.addToggleButton('media', {
            ...
            onSetup: buttonApi => {
                ...
                return selection.selectorChangedWithUnbind(
                    'img[data-mce-object],span[data-mce-object],div[data-ephox-embed-iri]', buttonApi.setActive).unbind;
            }
        });
