...
(function () {
    ...
    const equals = (lhs, rhs, comparator = tripleEquals) => lift2(lhs, rhs, comparator).
        getOr(lhs.isNone() && rhs.isNone());
    ...
    const isParentListSelected = (parentList, selectedBlocks) => isNonNullable(parentList) && selectedBlocks.length ===
        1 && selectedBlocks[0] === parentList;
    ...
    const isListHost = (schema, node) => !isListNode(node) && !isListItemNode(node) &&
    ...
    const isWithinNonEditable = (editor, element) => element !== null &&
    ...
    const getItemSelection = editor => {
        ...
        return lift2(find(selectedListItems, not(hasFirstChildList)),
            find(reverse(selectedListItems), not(hasFirstChildList)), (start, end) => ({
                start,
                end
            }));
    };
    ...
    const handleIndentation = (editor, indentation) => !selectionIsWithinNonEditableList(editor) &&
    ...
    const getSelectedTextBlocks = (editor, rng, root) => {
        ...
        global$1.each(siblings, node => {
            ...
            if (global.isBookmarkNode(node)) {
                if (isListNode(nextSibling) || isTextBlock(editor, nextSibling) || !nextSibling && node.parentNode ===
                    root) {
                    block = null;
                    return;
                }
    ...
    const backspaceDelete = (editor, isForward) => {
        ...
        return !isWithinNonEditableList(editor, selection.getNode()) &&
    ...
    const setupMenuButtonHandler = (editor, listName) => api => {
        const menuButtonHandler = e => api.setEnabled(
            inList(e.parents, listName) && !isWithinNonEditableList(editor, e.element));
